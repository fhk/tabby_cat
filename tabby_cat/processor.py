...
class Processor():
    ...
    def geom_to_graph(self):
        ...
        self.look_up = {k:covert_ids[v] for k, v in self.look_up.items() if v in largest_cc}
        self.edges = OrderedDict((convert_ids[k], convert_ids[v]) for k, v in self.edges.items() if k[0] in largest_cc)
