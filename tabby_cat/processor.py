...
class Processor():
    ...
    def geom_to_graph(self, rerun=False, traverse=3, node_gap=9, two_edge_cost=1000,
        ...
        if not rerun:
            ...
            self.flip_look_up = {v: k for k, v in self.look_up.items()}
            self.nodes_to_connect = set(self.look_up[k] for k, v in self.demand_nodes.items() if v)
        ...
        if not rerun:
            ...
            self.nodes_to_connect = set(self.convert_ids[n] for n in nodes_to_connect if n in largest_cc)
